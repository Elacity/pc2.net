...
### 5.3.2 Sibling-orchestrator type-only imports → Phase 2-D — **EXECUTED 2026-05-18 morning, AWAITING CI GREEN + REVIEW**

- **What**: same mechanical pattern as Phase 2-B, applied to the next batch — convert `import { AIChatService }` / `import { BosonService }` to `import type { ... }` at the 6 type-only consumer sites identified by the post-2-C survey (`server.ts`, `api/ai.ts`, `api/other.ts`, `gateway/ChannelBridge.ts`, `ContentIntelligenceService.ts`, `api/boson.ts`).
- **ROI**: resolves audit Pattern #1 (concrete-class consumer imports) for the sibling-orchestrator boundary; ~6 score points across 6 consumer modules; after this lands, the audit's mechanical-pattern blockers (concrete-class + ambient-singleton) are functionally exhausted for consumer modules in `pc2-node/src` (modulo the documented bootstrap and tightly-coupled internal-sibling exceptions, which the audit permits).
- **Effort**: ~50 min single-session (smaller than 2-B's 2h and 2-C's 3h — only 6 files touched).
- **Risk**: ≤ Phase 2-B level. Byte-identical compiled JS expected per file, except for one erased `require()` line per file (which is the actual audit-resolution evidence).
- **Honest scope-shrink**: the original audit text predicted a 3-hour "extract interfaces from sibling orchestrators" structural job. Post-2-C survey found that most consumer sites use the orchestrator class only as a type annotation — TypeScript's structural typing means `import type { AIChatService }` IS the interface, no nominal `IAIChatService` needed. Mechanical keyword change resolves the same blocker as nominal extraction would have.
- **Ticket**: see [`PHASE-2-D-SIBLING-ORCHESTRATOR-TYPE-ONLY.md`](./PHASE-2-D-SIBLING-ORCHESTRATOR-TYPE-ONLY.md) — file-by-file conversion table, documented "keep concrete" exceptions, SHA-256 validation strategy.
...
### 5.3.4 Phase 2-D-helpers (deep WASM consumer-site thread) — **EXECUTED 2026-05-18 afternoon, AWAITING CI GREEN + REVIEW**

- **Scope**: 7 route-chain `getWASMRuntime()` call sites in `api/storage.ts` (4) + `api/media.ts` (3) converted to threaded `wasmRuntime: WASMRuntime` parameters. 3 service-internal sites + 2 module-level preloads classified as intentional architectural-boundary ambient with explicit comments. 6 source files + 1 caller in `api/gateway.ts` updated.
- **Scope-shrink note**: Original audit said "8 deferred sites". Call-graph mapping found 10 sites; user-approved narrow scope converts 7, documents 3 as intentional. The 2 "module-level preloads" turned out not to use the WASM runtime at all (pure file reads), so the `getWASMRuntime` value-import was dropped entirely from `api/media.ts`.
- **Validation**: tsc clean first try (no compiler bugs surfaced this round — route-chain sites were already properly typed), build clean, test 7/7 in 52.2 ms, ReadLints 0 errors on 6 files, live `/api/health` smoke ✅.
- **Notable difference from 2-D**: compiled JS is NOT byte-identical (function signatures changed). Behavior equivalence is preserved by passing the exact same singleton through the parameter that ambient access would have returned (verified via bootstrap-time `app.locals.wasmRuntime = wasmRuntime`).
- **What this completes**: every route-chain consumer site in `pc2-node/src` now uses explicit dependency injection for WASMRuntime. The remaining 3 ambient sites are all properly classified as service-construction-time or bootstrap-time patterns — audit-permitted.

### 5.3.3 Ambient `global.*` cleanup → Phase 2-Globals — **EXECUTED 2026-05-18 midday, AWAITING CI GREEN + REVIEW**
...
- **Execution outcome (2026-05-18 midday)**: **all 4 steps applied, 7 files modified.** Pre-flight db survey of local install confirmed zero existing user resource-settings — bug-fix's user-visible blast radius is forward-looking only. **TypeScript caught 4 real pre-existing type bugs** during execution (db.getSetting returns `string | undefined` not `number`; db.setSetting accepts `string` not `number`) — these were silent under `any`-typed helpers and would have caused NaN propagation once the read path started working. Fixed via `parseInt()` on read + `String()` on write. All gates green: `tsc` clean, `build:backend` clean, `test:unit` 7/7 in 52.6ms, ReadLints 0 errors on 7 modified files. **This is the third Phase 2 phase where TypeScript caught real bugs during execution** (after 2-C's static-method and type-narrowing catches). Audit's mechanical-pattern blocker #2 is now functionally complete for consumer modules. Held behind Mac launcher soak gate before release-branch merge.
