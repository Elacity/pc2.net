...
> **Created:** 2026-02-24
> **Last Updated:** 2026-05-25 (three-pass: added Public Cloud PC2 — `try.pc2.net` — trial-host milestone under post-v1.2 strategic plan; D3 flipped from local Ollama to BYOK + waitlist CTA, unlocking new Phase 1.5 Elacity-hosted AI subscription tier — Elacity's first recurring-revenue product seed. **Pass 2 (same date)**: GoDaddy/DNS/cert audit session completed — Contabo `*.ela.city` cert refreshed (was 3 days expired; fresh through 2026-08-13), InterServer `backup-to-contabo.sh` patched so future acme.sh renewals auto-propagate to Contabo + reload its nginx, GoDaddy A record for `elastossmartchain.ela.city` repointed to Contabo. **Pass 3 correction (same evening)**: live application-layer probe found Contabo's nginx routes the new hostname to a wildcard `*.ela.city` user-profile backend (returns 404 "User not found" instead of the ESC RPC service). DNS + cert prereqs for SEC-WAVE6 A8 are landed, but **an additional nginx server block on Contabo is still required** before the A8 code patch can ship safely (~30–45 min server-side work, then ~30 min code patch). Recommended path: bundle the A8 nginx vhost with SUPERNODE-RPC-PROXY Phase 2's already-planned Contabo nginx changes — single reload, two new server blocks. Release snapshot unchanged. See `SEC-2026-04-22-WAVE6-HARDENING.md` §"2026-05-25 PM — A8 live-probe finding" + `SUPERNODE-RPC-PROXY.md` §"Suggested scope expansion".)
...
| **v1.3.0** *(active mandate)* | TBD — 8-12 eng-weeks from agentic spike start | 📋 Proposed | **Monetisation Agent Alpha** per [`AGENTIC-PC2-MONETISATION-2026-05`](../../.cursor/tasks/AGENTIC-PC2-MONETISATION-2026-05/AGENTIC-PC2-MONETISATION-2026-05.md). One JTBD: *"As a creator with files on my PC2 node, I want to say 'package my portfolio for sale' and have my agent walk me through the dDRM packaging flow, suggest pricing from comparable assets, draft license terms I can confirm in plain English, and publish to the Exchange."* Feature flag `agent.monetisation.alpha`. Extends existing `AIChatService` with a `monetisation-orchestrator` skill — no external dependencies on Particle UA V2, ERC-8004, or Runtime v2 capsules. S1 (Creator Studio dialogue + intent capture + Creator hand-off) is the in-progress slice — see [`AGENT-CREATOR-STUDIO-2026-05`](../../.cursor/tasks/AGENT-CREATOR-STUDIO-2026-05/PLAN.md). **Supersedes the older "blocked on Lit Chipotle + V3 + PDR" framing** — Lit Chipotle is now production-integrated (v1.2.7.4), V3 contracts went on-chain in v1.2.7.7 (`bulkUpdatePlans` + `configureTokenOwnershipAccess` + `subscribePlan`), PDR Phase B is no longer a v1.3 blocker. The original v1.3 work has either shipped or been re-scoped into later releases. **Status**: pending Sasha's sign-off on 6 open questions in §12 of the agentic mandate (branch strategy, feature flag, LLM choice, public protocol spec, security audit funding, manual-UI deprecation — all with recommended sensible defaults). |
| **Public Cloud PC2** (`try.pc2.net` — try-before-download) *(adoption-loop work, not release-bound)* | Planning landed 2026-05-25; eng start gated on Sasha sign-off | 📋 Proposed | **One dedicated VPS (Hetzner CCX33 ~€33/mo recommended), throwaway per-visitor Docker containers, sandboxed PC2 desktop in the browser.** ~30 concurrent / ~700/day capacity. **No production credentials in the trial image**; testnet Lit + testnet contracts + faucet-funded SA. Persistent sovereignty banner + Download CTA. Not colocated with supernodes. **Creator restricted to image+PDF+Markdown** (no ffmpeg on 0.5 vCPU → closes transcode-storm risk). **AI chat is BYOK** with an "Or get hosted AI from Elacity — coming soon" waitlist CTA captured during the trial itself — seeds the **future Phase 1.5 hosted-AI subscription tier** (€5–50/mo, LLM proxy at `llm.ela.city`, fans out to wholesale OpenRouter / OpenAI / Anthropic — see [`PLAN.md`](../../.cursor/tasks/PUBLIC-CLOUD-PC2-2026-05/PLAN.md) §9.4). Targets the install-friction gate at the top of the activation funnel. Full design + hosting comparison + rollout in [`.cursor/tasks/PUBLIC-CLOUD-PC2-2026-05/PLAN.md`](../../.cursor/tasks/PUBLIC-CLOUD-PC2-2026-05/PLAN.md). Fresh-agent handover at [`docs/handover/HANDOVER_2026-05-25_PUBLIC_CLOUD_PC2.md`](../handover/HANDOVER_2026-05-25_PUBLIC_CLOUD_PC2.md). **Status**: pending Sasha sign-off on 6 §0 decisions (hosting provider, isolation model, **AI = BYOK + waitlist**, auth model, chain access, domain). **Anticipated effort post sign-off**: ~8.5 eng-days, ~2.5 weeks calendar to public launch. |
| **dDRM Trust-Model Hardening** *(continuous track, not release-bound)* | Started 2026-05; multi-release | 🛡️ In-flight (owner: Irzhy) | Closes the **CEK-at-WASM-boundary** trust gap on operator-controlled PC2 nodes by moving CEK custody off the host process. **Tier 1 (paid non-media — PDFs, code, models, posts):** Lit Action streaming decryption inside the BLS-threshold TEE — CEK never observable on any client. Already in-flight per [`NONMEDIA-LIT-MIGRATION-2026-05`](../../.cursor/tasks/NONMEDIA-LIT-MIGRATION-2026-05/NONMEDIA-LIT-MIGRATION-2026-05.md) and supported by the Strategy-pattern refactor in `pc2-node/src/api/renderer/secure-view/` (commit `d2b5d479a`, see `pc2-node/docs/architecture/secure-view-renderer.md`). **Tier 2 (paid media — audio/video/images):** MPEG-DASH + Common Encryption (CENC) + browser-CDM via EME/Widevine/FairPlay — license server role for our Lit Action, decryption inside the browser's hardware-rooted CDM. Media packaging already aligned in commit `592a3be4f` per [`CENC_PACKAGING_COMPLIANCE`](./CENC_PACKAGING_COMPLIANCE.md) + [`MEDIA_DRM_PACKAGING`](./MEDIA_DRM_PACKAGING.md) + [`CHIPOTLE_V3_PROTOCOL`](./CHIPOTLE_V3_PROTOCOL.md); player-side EME integration still to spike. **Tier 3 (forensic watermarking, paid only):** per-buyer transcode with cryptographically derived watermark — leak-tracing layer; pre-spike. **Tier 0 (free / public content):** no DRM by design — IPFS + pin only. **What this track cannot solve (documented honesty, not theatre):** a determined local operator can always extract their own keys; the analog hole (screen-record / audio-loopback) always defeats prevention. We document this publicly rather than ship DRM theatre — a `docs/core/TRUST_MODEL.md` page is planned. **Monetisation Agent invariant**: the agent must never accept, return, log, or persist a CEK — enforced via JSDoc INVARIANT block in `pc2-node/src/services/ai/tools/MonetisationAgentTools.ts` and risk-register entry R13 in `AGENT-CREATOR-STUDIO-2026-05/PLAN.md` §11. |
...
| [PUBLIC-CLOUD-PC2-2026-05/PLAN.md](../../.cursor/tasks/PUBLIC-CLOUD-PC2-2026-05/PLAN.md) | **`try.pc2.net` design — try-before-download public-cloud PC2.** Architecture, hosting comparison (Hetzner / Contabo / InterServer / Vultr / DO / Linode), sizing math, isolation & sovereignty narrative, rollout plan. Status: Proposed, gated on Sasha sign-off of 6 decisions. |
| [HANDOVER_2026-05-25_PUBLIC_CLOUD_PC2.md](../handover/HANDOVER_2026-05-25_PUBLIC_CLOUD_PC2.md) | **Fresh-agent handover for the public-cloud PC2 task.** Copy-paste prompt for a new chat window, current state, sequencing, ops notes. |
...
**Tier 1.5 — Try-Before-Download Public Cloud PC2 (`try.pc2.net`)** *(added 2026-05-25, post-v1.2.8.0 tag)*:
- §T1.5.1 **Public trial host** — single dedicated VPS (Hetzner CCX33 ~€33/mo recommended) hosting throwaway per-visitor Docker containers, each running a sandboxed PC2 desktop. ~30 concurrent sessions / ~700/day capacity on the recommended hardware. Visitor lands on `try.pc2.net`, solves a captcha, gets a working PC2 desktop in their browser within ~8 seconds. Strict resource caps (512 MB RAM / 0.5 vCPU / 5 GB tmpfs per session), 60-min idle TTL, 4h hard cap. **No production credentials in the trial image** — testnet Lit, testnet contracts, faucet-funded throwaway SA per session. **Creator restricted to image + PDF + Markdown** (video/audio uploads refused with a "Download PC2 to mint videos" modal — closes the transcode-storm risk where 0.5 vCPU × 30 concurrent ffmpeg jobs would melt the host). **AI chat is BYOK** — no local Ollama, no Elacity-paid inference; visitor pastes their own provider key. When no key is configured, the chat tab surfaces an "Or get hosted AI from Elacity — coming soon" waitlist sign-up that captures revenue-interest signal during the trial itself. Sovereignty narrative kept intact via a persistent "Demo mode — your data is on Elacity's server, **Download PC2** for the real thing" banner on every UI panel. Persistent taskbar Download CTA is the conversion event. **Adoption-loop work, not engineering-loop work** — shortens the funnel from "interested" → "running PC2" by removing the install-first friction. Cost target: ~€35/mo all-in (BYOK = €0 LLM credits on us). Trial host is **NOT colocated** with our existing supernodes (Contabo 38.242.211.112 + InterServer 69.164.241.210) — separate provider to avoid blast-radius with production services. Full design + hosting comparison + sizing math + rollout plan in [`.cursor/tasks/PUBLIC-CLOUD-PC2-2026-05/PLAN.md`](../../.cursor/tasks/PUBLIC-CLOUD-PC2-2026-05/PLAN.md); standalone fresh-agent handover at [`docs/handover/HANDOVER_2026-05-25_PUBLIC_CLOUD_PC2.md`](../handover/HANDOVER_2026-05-25_PUBLIC_CLOUD_PC2.md). **Status: Proposed** — gated on Sasha sign-off of 6 §0 decisions (hosting provider, isolation model, AI = BYOK + waitlist, auth model, chain access, domain).
- §T1.5.1a **Phase 1.5 — Elacity-hosted AI subscription** *(parked, unlocked by §T1.5.1 BYOK design; standalone ticket later)* — when the trial-host BYOK waitlist captures meaningful interest (target ~200 sign-ups), stand up a small OpenAI-compatible LLM proxy at `llm.ela.city` that fans out to whichever wholesale LLM backend we negotiate the best rate with (OpenRouter, direct OpenAI, Anthropic). Per-subscriber Elacity-issued API key + per-tier rate limiting at the proxy. Pricing example (Sasha decides): **Tourist €5/mo · 100k tokens** / **Maker €15/mo · 500k tokens** / **Studio €50/mo · 2M tokens**. **Works on real PC2 and in the trial** — visitor pastes their Elacity AI key the same way they'd paste an OpenAI key. Wholesale LLM is typically 60–70 % of retail → comfortable margin even at €5/mo. Sovereignty narrative stays intact (BYOK still supported; subscription is convenience, not lock-in). Full design + pricing math + proxy architecture in [`PLAN.md`](../../.cursor/tasks/PUBLIC-CLOUD-PC2-2026-05/PLAN.md) §9.4. Will get its own ticket (`ELACITY-AI-SUBSCRIPTION-2026-XX`) when waitlist demand justifies build. **Strategic note**: this is Elacity's first recurring-revenue product tier — captured almost for free as a side-effect of the trial-host BYOK design.
- §T1.5.2 **Phase 2 — Hosted PC2 (paid, per-user dedicated VM)** *(parked, not in this task)* — real product: pay €5/mo, get a Hetzner Cloud CX22 (€4/mo to us, €1 margin) provisioned in your name with PC2 pre-installed, your wallet, your data. Provisioned via Hetzner Cloud API + cloud-init. User gets root SSH. Sovereignty narrative honoured because they have root and can `terraform destroy` us out of the picture any time. **Out of scope for this milestone**; will be a separate ticket if Phase 1 (`try.pc2.net`) shows meaningful conversion (≥ 1 %).
- §T1.5.3 **Phase 3 — DePIN Hosted (community-operated supernodes hosting PC2 instances)** *(parked, post-Phase-2)* — any community member with a beefy server becomes a "PC2 hosting provider" and earns ELA. Smart-contract-mediated. Lines up directly with [SUPERNODE_ECONOMICS.md](./SUPERNODE_ECONOMICS.md) supernode revenue model. Long-term endpoint of the sovereign-cloud trajectory.
