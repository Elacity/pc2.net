        ...
        if (aiService) {
            try {
                ...
                if (models.length > 0) {
                    ...
                    capabilities.vision = models.some(m => m.id.toLowerCase().includes('llava') ||
...
router.post('/config', authenticate, async (req, res) => {
    try {
        ...
        // Update config
        db.setAIConfig(walletAddress, newProvider, cleanModel, existing?.api_keys ? JSON.parse(existing.api_keys) : null, ollama_base_url || existing?.ollama_base_url || 'http://localhost:11434');
 ...
 */
router.post('/test-key', authenticate, async (req, res) => {
    try {
