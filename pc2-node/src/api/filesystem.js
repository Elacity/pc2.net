...
export function handleStat(req, res) {
    ...
    const uuidFromQuery = req.query.uuid;
    const uuidFromBody = req.body?.uuid;
...
export async function handleCopy(req, res) {
    ...
    // 1. { source: "/path", destination: "/path", newName: "..." } - Puter SDK format
    // 2. { from: "...", to: "..." } - Alternative format
    let sourcePath;
    let destPath;
    let newName;
    logger.info('[Copy] Request received', {
        bodyKeys: Object.keys(body || {}),
        source: body.source ? String(body.source).substring(0, 80) : undefined,
        destination: body.destination ? String(body.destination).substring(0, 80) : undefined,
        newName: body.newName || body.new_name
    });
    if (body.source) {
        ...
        res.status(400).json({ error: 'Missing source or destination', expected: '{ source: "/path", destination: "/path" }' });
        return;
    }
    if (!sourcePath || !destPath) {
        res.status(400).json({ error: 'Both source and destination are required' });
        return;
    }
    // Resolve paths (handle ~ and relative paths)
    ...
    // If newName is provided, append it to destPath (destPath is the target directory)
    let finalDestPath = destPath;
    if (newName) {
        // destPath is the directory, newName is the filename
        finalDestPath = destPath.endsWith('/') ? `${destPath}${newName}` : `${destPath}/${newName}`;
    } else {
        // No newName, use the source filename
        const sourceFileName = sourcePath.split('/').pop();
        if (destPath.endsWith('/') || !destPath.includes('.')) {
            // destPath is a directory
            finalDestPath = destPath.endsWith('/') ? `${destPath}${sourceFileName}` : `${destPath}/${sourceFileName}`;
        }
        // else destPath is already a full file path
    }
    logger.info('[Copy] Copying file', {
        source: sourcePath,
        dest: finalDestPath,
        wallet: req.user.wallet_address
    });
    try {
        const result = await filesystem.copyFile(sourcePath, finalDestPath, req.user.wallet_address);
        // Emit WebSocket event for real-time update
        if (io) {
            const room = `user:${req.user.wallet_address}`;
            io.to(room).emit('item.added', {
                uid: `uuid-${finalDestPath.replace(/\//g, '-')}`,
                uuid: `uuid-${finalDestPath.replace(/\//g, '-')}`,
                name: finalDestPath.split('/').pop(),
                path: finalDestPath,
                dirpath: finalDestPath.substring(0, finalDestPath.lastIndexOf('/')),
                is_dir: result.is_dir || false
            });
        }
        // Return response in Puter SDK expected format
        const response = [{
            copied: {
                uid: `uuid-${finalDestPath.replace(/\//g, '-')}`,
                path: finalDestPath,
        ...
        logger.info('[Copy] File copied successfully', { source: sourcePath, dest: finalDestPath });
        res.json(response);
    } catch (error) {
        ...
        if (error instanceof Error && error.message.includes('not found')) {
            ...
            res.status(409).json({ error: 'item_with_same_name_exists', message: error.message });
        } else {
            res.status(500).json({
