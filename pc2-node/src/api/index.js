...
export function setupAPI(app) {
    ...
    app.get('/api/user/login-history', authenticate, async (req, res) => {
        try {
            if (!req.user) {
                return res.status(401).json({ error: 'Unauthorized' });
            }
    ...
    app.get('/auth/list-sessions', authenticate, async (req, res) => {
        try {
            if (!req.user) {
                return res.status(401).json({ error: 'Unauthorized' });
            }
    ...
    app.post('/auth/revoke-session', authenticate, async (req, res) => {
        try {
            if (!req.user) {
                return res.status(401).json({ error: 'Unauthorized' });
            }
