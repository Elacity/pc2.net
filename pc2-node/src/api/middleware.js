...
export function authenticate(req, res, next) {
    ...
    if (!session && (token === 'mock-token' || token.startsWith('mock-token'))) {
        ...
        const pathToCheck = req.query.uid || // ✅ Check uid param first (contains UUID with wallet for writeFile)
            req.query.file || // ✅ Check file param (most common for read operations)
            req.query.path ||
            req.body?.file ||
            req.body?.path ||
            req.path;
