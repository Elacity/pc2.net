...
export function handleDriversCall(req, res) {
    ...
    if (typeof body === 'string') {
        try {
            ...
            req.body = body; // Update req.body for consistency
        }
        catch (e) {
    ...
    if (isEmptyBody) {
        ...
        if (appNameFromQuery) {
            ...
            const appMap = {
                ...
                'app-center': { name: 'app-center', title: 'dApp Centre', uuid: 'app-app-center', uid: 'app-app-center', icon: undefined, index_url: `${baseUrl}/apps/app-center/index.html` },
                    ...
                    if (method === 'complete') {
                        ...
                        if (stream) {
                            ...
                            try {
                                ...
                                let filesystem = req.app.locals.filesystem;
                                const walletAddress = req.user?.wallet_address;
                                ...
                                if (!filesystem) {
                                    ...
                                    const globalFilesystem = global.__filesystem;
                                    if (globalFilesystem) {
                                ...
                                if (walletAddress) {
                                    await aiService.registerUserProviders(walletAddress);
                                }
                                    ...
                                    if (chunk.message?.tool_calls && Array.isArray(chunk.message.tool_calls)) {
                                        for (const toolCall of chunk.message.tool_calls) {
                                            try {
                                                ...
                                                const toolSource = toolCall.__source;
                                                const toolChunk = {
                                                    type: 'tool_use',
                                                    name: toolName,
                                                    input: args,
        ...
        if (db && typeof db.listFiles === 'function') {
            ...
            if (!existingMetadata) {
                logger.warn('[WriteFile] UUID not found in database', {
                    ...
                    sampleUuids: allFiles.slice(0, 3).map((f) => `uuid-${f.path.replace(/\//g, '-')}`)
                });
            }
        }
    }
 ...
 * POST /set-desktop-bg
 */
export function handleSetDesktopBg(req, res) {
    const db = req.app.locals.db;
    const body = req.body;
    if (!db) {
        res.status(500).json({ error: 'Database not initialized' });
 ...
 */
export function handleSetProfilePicture(req, res) {
    const db = req.app.locals.db;
    const body = req.body;
    if (!db) {
        res.status(500).json({ error: 'Database not initialized' });
