...
export function createServer(options) {
        ...
        if (contentType.includes('text/plain') && contentType.includes('actually=json') && typeof req.body === 'string') {
            try {
                ...
                if (req.body && req.body.trim().length > 0) {
                    ...
                    req.rawBody = req.body; // Store raw for debugging
                    req.body = parsed; // Replace string with parsed object
                }
                else {
    ...
    if (options.filesystem) {
        ...
        global.__filesystem = options.filesystem;
    ...
    const terminalConfig = options.config?.terminal || {};
