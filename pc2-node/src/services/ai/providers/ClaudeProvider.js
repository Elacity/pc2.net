...
export class ClaudeProvider {
    anthropic;
    defaultModel = 'claude-sonnet-4-5-20250929'; // Current Claude Sonnet 4.5 model
    constructor(config) {
        if (!config?.apiKey) {
    ...
    async complete(args) {
        ...
        try {
            ...
            if (args.tools && args.tools.length > 0) {
                sdkParams.tools = args.tools.map((tool) => ({
                    name: tool.function?.name || tool.name,
                    description: tool.function?.description || tool.description,
                    input_schema: tool.function?.parameters || tool.parameters || {},
                }));
    ...
    async *streamComplete(args) {
        ...
        try {
            ...
            let toolCalls = [];
            let currentToolCallBuffer = ''; // Buffer for current tool call JSON
            let currentToolCallInfo = null; // Current tool call being built
