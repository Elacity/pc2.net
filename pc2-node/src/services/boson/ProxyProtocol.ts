 ...
 * 
 * Java server uses: base = ordinal * 8, ACK = base | 0x80
 * Lower 3 bits are randomized by server (e.g., PONG can be 0x90-0x97)
 * 
 * Confirmed wire evidence:
 *   AUTH_ACK = 0x80-0x87 (confirmed from working AUTH)
 *   PONG     = 0x90-0x97 (wire 0x92 observed)
 *   CONNECT_ACK = 0x98-0x9F (wire 0x9f observed)
 * 
 * Derived non-ACK bases (ordinal * 8):
 *   AUTH=0x00, ATTACH=0x08, PING=0x10, CONNECT=0x18, 
 *   DISCONNECT=0x20, DATA=0x28, ERROR=0x30
 */
export enum PacketType {
  // Authentication: base 0x00, wire 0x00-0x07
  AUTH = 0x00,
  // AUTH_ACK: base 0x80, wire 0x80-0x87
  AUTH_ACK = 0x80,
  // AUTH_ERROR: treated same as ERROR (0x30-0x37 or 0x70-0x7F)
  AUTH_ERROR = 0x02,
  
  // Session attachment: base 0x08, wire 0x08-0x0F
  ATTACH = 0x08,
  // ATTACH_ACK: base 0x88, wire 0x88-0x8F
  ATTACH_ACK = 0x88,
  ATTACH_ERROR = 0x0A,
  
  // Keep-alive: PING base 0x10 (wire 0x10-0x17), PONG base 0x90 (wire 0x90-0x97)
  PING = 0x10,
  PONG = 0x90,
  
  // Connection management: CONNECT base 0x18 (wire 0x18-0x1F), ACK base 0x98 (wire 0x98-0x9F)
  CONNECT = 0x18,
  CONNECT_ACK = 0x98,
  
  // Disconnection: base 0x20 (wire 0x20-0x27), ACK base 0xA0 (wire 0xA0-0xA7)
  DISCONNECT = 0x20,
  DISCONNECT_ACK = 0xA0,
  
  // Data transfer: base 0x28, wire 0x28-0x2F
  DATA = 0x28,
  
  // Errors: base 0x30, wire 0x30-0x37
  ERROR = 0x30,
}
