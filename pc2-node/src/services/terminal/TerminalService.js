 ...
 */
function checkBubblewrapAvailable() {
    try {
...
export class TerminalService {
     ...
     */
    determineEffectiveIsolationMode() {
     ...
     */
    createSafeEnvironment(walletAddress, userHome) {
        const env = {};
     ...
     */
    buildBwrapCommand(shell, userHome, env) {
        const bwrapArgs = [
            // Unshare namespaces
            '--unshare-all',
     ...
     */
    async createSession(walletAddress, cols = 80, rows = 24) {
        // Check if terminal is available
        if (!this.isAvailable()) {
            return {
                sessionId: '',
     ...
     */
    getSession(sessionId, walletAddress) {
     ...
     */
    write(sessionId, walletAddress, data) {
     ...
     * Resize a terminal
     */
    resize(sessionId, walletAddress, cols, rows) {
        const session = this.getSession(sessionId, walletAddress);
        if (!session) {
            return false;
        }
     ...
     */
    onData(sessionId, walletAddress, callback) {
        const session = this.getSession(sessionId, walletAddress);
        if (!session) {
            return false;
        }
     ...
     */
    onExit(sessionId, walletAddress, callback) {
        const session = this.getSession(sessionId, walletAddress);
        if (!session) {
            return false;
        }
     ...
     */
    destroySession(sessionId, walletAddress) {
        const normalizedWallet = walletAddress.toLowerCase();
        const session = this.sessions.get(sessionId);
        if (!session) {
            return false;
     ...
     */
    destroyAllUserSessions(walletAddress) {
        const normalizedWallet = walletAddress.toLowerCase();
        const sessionIds = this.userSessions.get(normalizedWallet);
        if (!sessionIds) {
            return 0;
     ...
     * Audit logging
     */
    logAudit(action, walletAddress, sessionId, details) {
        const logEntry = {
...
// Singleton instance
let terminalServiceInstance = null;
export function getTerminalService(config) {
    if (!terminalServiceInstance) {
        terminalServiceInstance = new TerminalService(config);
    }
    return terminalServiceInstance;
}
export function shutdownTerminalService() {
    if (terminalServiceInstance) {
