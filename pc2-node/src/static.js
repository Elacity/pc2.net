...
export function setupStaticServing(app, options) {
    ...
    app.get('/puter.js/v2', async (req, res, next) => {
...
`;
    ...
    function appsRouteHandler(req, res, next) {
    ...
    <script>
        ...
        window.addEventListener('unhandledrejection', (e) => {
            console.error('[Terminal] Unhandled promise rejection:', e.reason);
                        ...
                        htmlContent = htmlContent.replace(/(<script src="[^"]*puter\.js\/v2"><\/script>)/, `$1${sdkInitScript}`);
                        // Also enhance the main_term initialization with better error handling
                        htmlContent = htmlContent.replace(/if \(typeof puter === 'undefined'\) \{[^}]*return;[^}]*\}/s, `if (typeof puter === 'undefined') {
    ...
    <script>
        ...
        window.addEventListener('unhandledrejection', (e) => {
            ...
            if (indicator) indicator.textContent = '[Phoenix] ❌ Unhandled rejection: ' + String(e.reason);
                        ...
                        htmlContent = htmlContent.replace(/if \(typeof puter === 'undefined'\) \{[\s\S]*?return;[\s\S]*?\}/, `if (typeof puter === 'undefined') {
