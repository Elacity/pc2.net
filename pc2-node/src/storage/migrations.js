    ...
    if (currentVersion < CURRENT_VERSION) {
        ...
        if (currentVersion < 6) {
            try {
                ...
                const rows = db.prepare('SELECT wallet_address, default_model FROM ai_config WHERE default_model IS NOT NULL').all();
                let cleaned = 0;
        ...
        if (currentVersion < 7) {
            try {
                ...
                for (const oldModel of deprecatedModels) {
                    const rows = db.prepare('SELECT wallet_address, default_model FROM ai_config WHERE default_model = ?').all(oldModel);
                    for (const row of rows) {
