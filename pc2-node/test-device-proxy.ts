 ...
 * 
 * Run this on your Raspberry Pi, Jetson Nano, or any device to test
 * Active Proxy connectivity to the Boson supernode.
 * 
 * Usage:
 *   npx tsx test-device-proxy.ts
 * 
 * What it tests:
 *   1. Generates a test identity (Ed25519 keypair)
 *   2. Connects to the supernode via Active Proxy
 *   3. Completes the cryptographic handshake
 *   4. Reports the allocated public port
 */
...
const SUPERNODE = {
  ...
  port: 8090,
  // Ed25519 public key (node ID) from the supernode
...
async function testActiveProxy() {
  ...
  return new Promise<void>((resolve, reject) => {
    ...
    let state: 'connecting' | 'challenge' | 'auth_sent' | 'done' = 'connecting';
    ...
    socket.on('data', (data) => {
      ...
      if (state === 'challenge') {
        // Parse challenge: [2-byte length][random bytes]
        if (dataBuffer.length < 2) return;
        const challengeLen = dataBuffer.readUInt16BE(0);
        if (dataBuffer.length < challengeLen) return;
        ...
        authPacket.writeUInt8(0x00, 2); // AUTH type
      ...
      if (state === 'auth_sent') {
        // Parse AUTH_ACK: [2-byte length][1-byte type (0x80-0x87)][cipher][padding]
        if (dataBuffer.length < 3) return;
        const responseLen = dataBuffer.readUInt16BE(0);
        if (dataBuffer.length < responseLen) return;
        ...
        if (isAuthAck) {
          // Decrypt AUTH_ACK (51 bytes cipher = 35 plaintext + 16 MAC)
          ...
          // Parse: [32-byte serverPk][2-byte port][1-byte domain]
          ...
          // Keep connection alive for a moment then close
          setTimeout(() => {
            socket.destroy();
            resolve();
          }, 2000);
