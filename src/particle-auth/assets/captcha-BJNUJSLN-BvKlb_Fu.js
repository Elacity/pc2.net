...
`,Y=a.div`
  ...
  pointer-events: none;
  z-index: 2;
`}}),Z,z,F,ee,De=m({"src/components/codeInput/index.tsx"(){$e(),Z=["alpha","numeric","alphanumeric"],z={alpha:{type:"text",inputMode:"text",pattern:"[a-zA-Z]{1}"},alphanumeric:{type:"text",inputMode:"text",pattern:"[a-zA-Z0-9]{1}"},numeric:{type:"tel",inputMode:"numeric",pattern:"[0-9]{1}",min:"0",max:"9"}},F=d.forwardRef(({allowedCharacters:i="alphanumeric",ariaLabel:c,autoFocus:s=!0,disabled:g,isPassword:y=!1,length:v=6,placeholder:C,onChange:b,code:u="",error:j=""},_)=>{if(isNaN(v)||v<1)throw new Error("Length should be a number and greater than 0");if(!Z.some(r=>r===i))throw new Error("Invalid value for allowedCharacters. Use alpha, numeric, or alphanumeric");const[S,p]=$.useState(0),[k,M]=$.useState(!1),n=d.useRef([]),E=z[i],h=d.useRef(null);d.useImperativeHandle(_,()=>({focus:()=>{var r;n.current&&((r=n.current[0])==null||r.focus())},clear:()=>{var r;if(n.current){for(let e=0;e<n.current.length;e++)(r=n.current)!=null&&r[e]&&(n.current[e].value="");n.current[0].focus()}o()}})),d.useEffect(()=>{var r;if(u){for(let e=0;e<n.current.length;e++)n.current[e].value="";for(let e=0;e<u.length;e++)(r=n.current)!=null&&r[e]&&(n.current[e].value=u[e]||"");o()}},[u]);let I=n.current.map(r=>r.value).join("")||"";const o=()=>{const r=n.current.map(e=>e.value).join("");I!==r&&(r.length===6&&setTimeout(()=>{n.current.forEach(e=>{e.blur()})},400),b&&b(r),I=r)},f=r=>{var w;const{target:{value:e,nextElementSibling:l}}=r;if(e.length==1)e.match(E.pattern)?l!==null&&l.focus():r.target.value="";else if(e.length>1){const R=e;let x=Number(r.target.getAttribute("data-index"))||0;for(let P=0;P<R.length;P++){const T=R.charAt(P);T.match(E.pattern)&&((w=n.current)!=null&&w[x])&&(n.current[x].value=T,n.current[x].nextElementSibling!==null&&(n.current[x].nextElementSibling.focus(),x++))}}setTimeout(()=>{o()})},he=r=>{const{key:e}=r,l=r.target;if(e==="Backspace"){if(l.value===""){if(l.previousElementSibling!==null){const w=l.previousElementSibling;w.value="",w.focus(),r.preventDefault()}}else l.value="";setTimeout(()=>{o()})}},fe=r=>{r.target.select()},xe=r=>{let e=r.clipboardData.getData("Text")??"";e=e.replace(/\s/g,"").substring(0,6);let l=S;for(let w=0;w<e.length;w++){const R=e.charAt(w),x=n.current[l];if(!x)break;const P=x==null?void 0:x.value;R.match(E.pattern)&&(P||(n.current[l].value=R,n.current[l].nextElementSibling!==null&&(n.current[l].nextElementSibling.focus(),l++)))}o(),r.preventDefault()};return t.jsxs(X,{ref:h,children:[t.jsx(Y,{$focusIndex:S,$showFocusBorder:k}),new Array(6).fill(0).map((r,e)=>t.jsx(Q,{"data-index":e,onChange:f,onKeyDown:he,onFocus:l=>{M(!0),p(e),fe(l)},autoFocus:s&&e===0,onBlur:()=>{M(!1)},onPaste:xe,...E,type:y?"password":E.type,ref:l=>{n.current[e]=l},autoComplete:e===0?"one-time-code":"off","aria-label":c?`${c}. Character ${e+1}.`:`Character ${e+1}.`,disabled:g,placeholder:C,error:j},e)),t.jsx(J,{onClick:r=>{r.preventDefault(),r.stopPropagation();for(let e=n.current.length-1;e>=0;e--)if(e>0&&!n.current[e].value&&n.current[e-1].value||e===0&&!n.current[e].value||e==n.current.length-1&&n.current[e].value){n.current[e].focus(),n.current[e].setSelectionRange(n.current[e].value.length,n.current[e].value.length);break}}})]})}),ee=F}}),te,ne,Ae=m({"src/components/loadingSpinner/styles.ts"(){te=a(V.div)`
  position: relative;
  width: 100%;
  height: 100%;
  display: flex;
...
`}}),H,re,Ze=m({"src/components/loadingSpinner/index.tsx"(){Ae(),H=({size:i="50%",borderColor:c="#a748ff",borderWidth:s=3,spin:g=!1,children:y})=>t.jsxs(te,{initial:{opacity:0},animate:{opacity:1},exit:{opacity:0},children:[g&&t.jsx(t.Fragment,{children:[...Array(4)].map((v,C)=>t.jsx(ne,{width:i,height:i,style:{animationDelay:`-${C*.15}s`,borderWidth:s,borderColor:`${c} transparent transparent transparent`}},C))}),t.jsx("div",{style:{opacity:g?.05:1},children:y})]}),re=H}}),ie,ae,ze=m({"src/components/reSend/styles.ts"(){ie=a.div`
  text-align: center;
  margin-top: 40px;
...
`,ae=a.div`
  ...
  position: relative;
  height: 24px;
`}}),L,B,oe,Fe=m({"src/components/reSend/index.tsx"(){N(),O(),Ze(),ze(),L=60,B=({reSendCode:i})=>{const c=K(),[s,g]=d.useState(0),{showError:y}=q(),[v,C]=d.useState(L),[b,u]=d.useState(!1),j=d.useRef(null),_=()=>{C(L),g(1),j.current=setInterval(()=>{C(p=>{const k=p-1;return k===0&&(g(2),clearInterval(j.current)),k})},1e3)},S=async()=>{try{if(b)return;u(!0),s===2&&(await i(),_())}catch(p){y((p==null?void 0:p.message)||"")}setTimeout(()=>{u(!1)},100)};return d.useEffect(()=>(_(),()=>{clearInterval(j.current)}),[]),t.jsx(ie,{children:s!==0&&t.jsxs("div",{children:[c.didNotReceiveCode,t.jsx(ae,{$pointer:s===2,children:s===1||s===3?`${v} s`:s===2?t.jsxs(t.Fragment,{children:[b&&t.jsx("div",{style:{width:"24px",height:"24px",top:"1px",position:"relative"},children:t.jsx(re,{spin:!0,borderWidth:2})}),t.jsx("div",{onClick:S,children:c.resendCode})]}):""})]})})},oe=B}}),se,le,ce,pe,de,ue,He=m({"src/pages/captcha/styles.ts"(){se=a.div`
  position: relative;
  padding-top: 10px;
`,a.div`
  position: relative;
...
`}}),W,Be,We=m({"src/pages/captcha/index.tsx"(){Te(),Le(),we(),De(),ve(),Fe(),ye(),be(),je(),_e(),N(),O(),Se(),me(),He(),W=({value:i,type:c})=>{const s=K(),{config:g}=Ee(),{error:y,showError:v}=q(),C=Ie(ke(g.chains[0])?"particleEVM":"particleSolana"),{connectAsync:b}=Re(),{appearance:u}=Pe(),[j,_]=d.useState(!1),S=d.useRef(null),p=ge("particleAuth"),{navigate:k}=Me(),M=async h=>{await b({connector:C,authParams:h})},n=async h=>{var I;try{if(j)return;if(h.length===6){if(/![^0-9]/.test(h))throw new Error(s.incorrectCode);{_(!0);const o={code:h,...c==="phone"?{phone:i}:{email:i}};if(o.code){if(!o.phone&&!o.email)throw new Error("Please enter the phone number or email")}else throw new Error("Please enter the verification code");await M(o)}}else v("")}catch(o){let f=((I=o==null?void 0:o.extra)==null?void 0:I[0])||o.message||(typeof o=="string"?o:s.networkError);f.includes("Invalid code")?f=s.invalidCode:f.includes("Code is deleted")?f=s.codeDeleted:f.includes("Network Error")&&(f=s.networkError),o.code===4011?(k("signup"),setTimeout(()=>v(f))):v(f)}_(!1)},E=async()=>{let h="";c==="phone"&&(h=await(p==null?void 0:p.auth.getCloudflareTurnstileResponse({theme:u.mode||"lgiht",language:u.language||"en",getContainer:S.current})));const I=c==="phone"?{phone:i,cf_turnstile_response:h}:{email:i};await(p==null?void 0:p.auth.getConnectCaptcha(I))};return t.jsx(Ce,{style:{padding:"12px 2px 8px"},children:t.jsxs(se,{ref:S,children:[t.jsx(pe,{children:t.jsx(Ve,{width:70,height:70,inset:2,logo:t.jsx(ue,{children:c==="phone"?t.jsx(G,{}):t.jsx(U,{})})})}),t.jsxs(de,{children:[t.jsx(le,{children:s.verificationCode}),t.jsx(ce,{children:i})]}),t.jsx(ee,{allowedCharacters:"numeric",length:6,placeholder:" ",onChange:n,error:y||""}),t.jsx(oe,{reSendCode:E})]})})},Be=W}});We();export{Be as default};
